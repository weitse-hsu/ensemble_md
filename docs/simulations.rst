...
.. _`Tutorial 1: Launching a REXEE simulation`: examples/tutorial_1/standard_REXEE.ipynb
...
----------------
  ...
  - :code:`modify_coords`: (Optional, Default: :code:`None`)
      ...
      Modules/functions not obeying the contract are unlikely to work.
        ...
        - The function must save the modified GRO file as :code:`confout.gro`. Specifically, if :code:`directory_A/output.gro` and :code:`directory_B/output.gro` are input, then :code:`directory_A/confout.gro` and :code:`directory_B/confout.gro` must be saved. (For more information, please visit `Tutorial 2: Multi-topology REXEE (MT-REXEE) simulations`_.) Note that in the CLI :code:`run_REXEE`, :code:`confout.gro` generated by GROMACS will be automatically renamed with a :code:`_backup` suffix to prevent overwriting.

.. _`Tutorial 2: Multi-topology REXEE (MT-REXEE) simulations`: examples/tutorial_2/MT_REXEE.ipynb
