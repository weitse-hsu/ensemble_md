...
def _parse_mc_lambda_row(fields):
    """
    Extracts the Count and G(in kT) values from one row of an expanded-ensemble "MC-lambda
    information" table. A row looks like
    "<row index> <lambda_1> ... <lambda_k> <Count> <G(in kT)> [<dG(in kT)>] [<<]" - the number
    of leading lambda columns (k) varies by how many lambda types are being scanned, and
    whether a trailing dG value is present varies by both the GROMACS version and, in the
    versions that do print it, whether this row is the last listed state in the table.
    Neither the row index nor Count is ever printed with a decimal point, whereas every
    lambda value, G, and dG always is (GROMACS prints even a value like 0.000 with one) -
    that lets us locate Count as the second integer-looking token (the first is always the
    row index) regardless of how many columns come before or after it.

    Parameters
    ----------
    fields : list
        The whitespace-split tokens of one data row, e.g. from :code:`line.split()`.

    Returns
    -------
    count : int
        The histogram count for this state.
    weight : float
        The G(in kT) value (weight) for this state.
    """
    if fields[-1] == "<<":
        fields = fields[:-1]
    int_positions = [idx for idx, tok in enumerate(fields) if "." not in tok]
    if len(int_positions) < 2:
        raise ParseError(f"Failed to parse MC-lambda row {fields}: could not identify the 'Count' column.")  # noqa: E501
    count_idx = int_positions[1]  # the first integer-looking token is always the row index
    return int(fields[count_idx]), float(fields[count_idx + 1])
        ...
        for l in lines:  # noqa: E741
            ...
            if "Count   G(in kT)" in l:
                ...
                for i in range(1, N_states + 1):
                    count, weight = _parse_mc_lambda_row(lines[n - i].split())
                    counts.append(count)
                    w.append(weight)
                weights.append(w)
                break
